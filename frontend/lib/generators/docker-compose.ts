...
/**
 * Generate docker-compose.yml for Supabase services
 */
export function generateDockerCompose(
  config: GeneratedConfig,
  options?: { enable_vector?: boolean; enable_logflare?: boolean; enable_pgvector?: boolean }
): string {
  const { enable_vector = false, enable_logflare = false, enable_pgvector = false } = options || {};

  const vectorService = enable_vector ? `
  vector:
    ...
    volumes:
      - ./volumes/logs:/var/log:ro
      - ./volumes/logs/vector.yml:/etc/vector/vector.yml:ro
    ports:
      - "8686:8686"` : '';

  const logflareService = enable_logflare ? `
  logflare:
    image: logflare/logflare:latest
    ports:
      - "4000:4000"
    environment:
  ...
  const dbExtensions = enable_pgvector ? `
      - CREATE EXTENSION IF NOT EXISTS vector;
      - CREATE EXTENSION IF NOT EXISTS pgvector;` : '';

  return `# Docker Compose configuration for Supabase
# Generated on ${new Date().toISOString()}
# Project: ${config.project_name}

services:
  studio:
    image: supabase/studio:20231123-a0ce836
  ...
  rest:
    ...
    restart: unless-stopped
    environment:
      PGRST_DB_URI: postgres://supabase_admin:\${POSTGRES_PASSWORD}@db:5432/postgres
  ...
  vector:
    ...
    command: ["--config", "etc/vector/vector.yml"]${vectorService}${logflareService}
